...
@AndroidEntryPoint
abstract class BaseOilFragment : BaseFragment<FragmentOilAvgBinding>(R.layout.fragment_oil_avg) {
    ...
    abstract fun getOilKindString(): Int
    abstract fun getOilCode(): Int
    ...
    private fun setupUI() {
        binding.oilKind.text = getString(getOilKindString())
        binding.oilAvgRecyclerView.adapter = oilAvgAdapter
    }

    private fun setupChart() {
        binding.lineChart.apply {
            xAxis.valueFormatter = IndexAxisValueFormatter(dateConverter.getRecentWeekDays(requireContext()))
    ...
    private fun fetchOilData() {
        oilAvgViewModel.fetchOilAvg(getString(getOilCode()))
    }
    ...
    private fun updateCurrentPrice(oilAvgPriceInfoList: List<OilAveragePriceInfo>) {
        if (oilAvgPriceInfoList.isNotEmpty()) {
            binding.priceText.text = RidRoundMath.roundStringToInteger(oilAvgPriceInfoList.last().oilPrice).toString()
        }
    }
} 